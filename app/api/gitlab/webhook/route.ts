...
export async function POST(req: Request){
  ...
  if(evt.object_kind === 'merge_request' && ['open','reopen','update'].includes(evt.object_attributes?.action)){
    ...
    const meta = { provider: 'gitlab', mr: evt.object_attributes?.iid, class: 'quick' };
    const job = await prisma.reviewJob.create({ data: { repoUrl, status: 'queued', meta: JSON.stringify(meta) } });
    const parsedJob = {
      ...job,
      meta: job.meta ? JSON.parse(job.meta) : null,
    };
    return NextResponse.json({ ok:true, job: parsedJob });
  }
